...
function repeatString(str, times) {
  return times && times > 0 ? str.repeat(times) : '';
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < words.length; i += 1) {
    const word = words[i];
    const formattedWord = word.replace(/[^a-z0-9]/gi, '');
    if (formattedWord.length > longestLength) {
      longestWord = formattedWord;
      longestLength = formattedWord.length;
    }
  }
  return longestWord;
}
...
function extractNameFromTemplate(value) {
  const namePart = value.slice(7);
  const name = namePart.slice(0, -1);
  return name;
}
...
function unbracketTag(str) {
  const tagWithoutOpeningBracket = str.slice(1);
  const tagWithoutClosingBracket = tagWithoutOpeningBracket.slice(0, -1);
  return tagWithoutClosingBracket;
}
...
function extractEmails(str) {
  const emailSubstrings = str.split(';');
  const validEmailSubstrings = emailSubstrings.filter(
    (substring) => substring.trim() !== ''
  );
  const trimmedEmails = validEmailSubstrings.map((substring) =>
    substring.trim()
  );
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (/[A-Za-z]/.test(str[i])) {
      const baseCharCode =
        str[i] >= 'a' ? 'a'.charCodeAt(0) : 'A'.charCodeAt(0);
